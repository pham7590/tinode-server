...
var httpConfig ahttp.ConnectionConfig
var adp adapter.Adapter
var db *testDB
var arango_db driver.Database
...
func (a *testDB) QueryOnef(
	queryString string,
	answer interface{},
	args ...interface{},
) error {
	formatted := fmt.Sprintf(queryString, args...)
	cursor, err := a.Query(context.Background(),
		formatted,
		nil,
	)
	if err != nil {
	...
	defer cursor.Close()
	_, err = cursor.ReadDocument(context.Background(), answer)
	if err != nil {
		return err
	}
	return nil
...
func TestTopicCreateP2P(t *testing.T) {
	...
	_, err = db.subscriptions.ReadDocument(ctx, subs[2].Id, &got)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUserUpdate(t *testing.T) {
	...
	var got types.User
	_, err = db.users.ReadDocument(ctx, users[0].Id, &got)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCredFail(t *testing.T) {
	...
	_ = db.QueryOnef(`for d in credentials
		filter d.User == "%s" && d.Method == "tel" && d.Value == "%s"
		return d
		`, &got, creds[3].User, creds[3].Value)
	if got.Retries != 1 {
...
func TestCredConfirm(t *testing.T) {
	...
	got = types.Credential{}
	// and uncomfirmed credential deleted
	_, err = db.credentials.ReadDocument(ctx, creds[3].User+":"+got.Method+":"+got.Value, &got)
...
func TestTopicUpdateOnMessage(t *testing.T) {
	...
	var got types.Topic
	_, err = db.topics.ReadDocument(ctx, topics[2].Id, &got)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTopicUpdate(t *testing.T) {
	...
	var got types.Topic
	_, _ = db.topics.ReadDocument(ctx, topics[0].Id, &got)
...
func TestSubsUpdate(t *testing.T) {
	...
	_, _ = db.subscriptions.ReadDocument(ctx, topics[1].Id, &got)
	if got.UpdatedAt != update["UpdatedAt"] {
		t.Errorf(mismatchErrorString("UpdatedAt", got.UpdatedAt, update["UpdatedAt"]))
	}
}
...
func TestDeviceDelete(t *testing.T) {
	...
	_, err = db.users.ReadDocument(ctx, users[2].Id, &got)
	if err != nil {
		t.Fatal(err)
	}
	if len(got.Devices) != 0 {
...
func TestCredDel(t *testing.T) {
	...
	cur, err := db.Query(ctx, `for d in users
	filter d.Method=="email" && d.Value=="alice@test.example.com"
	return d`, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	cur, err = db.QueryManyf(`for d in credentials
	filter d.User == "%s"
	return d`, users[1].Id)
	if err != nil {
		t.Fatal(err)
...
func initConnectionToDb() {
	...
	var err error
	connOpts.Endpoints = []string{"http://localhost:8529"}
	conn, err := ahttp.NewConnection(connOpts)
	if err != nil {
		log.Panicln(err)
	}
	...
	client.CreateDatabase(ctx, "test-chat", &driver.CreateDatabaseOptions{})
	arango_db, err = client.Database(ctx, "test-chat")
	if err != nil {
		log.Println(err)
	}
	db = &testDB{Database: arango_db}
	collections := []string{
		"kvmeta",
		"users",
		"auth",
	...
	for _, v := range collections {
		...
		coll, err := db.Collection(ctx, v)
		if err != nil {
			if driver.IsNotFound(err) {
				coll, err = db.CreateCollection(ctx, v, collectionOptions)
				if err != nil {
					log.Println(err)
				}
			} else {
